#!/usr/bin/env python3
"""Atomically update only the activated AI workload's immutable image fields."""
...
EXPECTED_REPOSITORY = "ghcr.io/team-pinlog/ai"
...
def parse(path: Path) -> tuple[
    ...
    if document.get("application") != {"enabled": True}:
        raise ValueError("AI application must remain enabled during image updates")
    if document.get("deployment") != {"enabled": True}:
        raise ValueError("AI deployment must remain enabled during image updates")
    if document.get("bootstrap", {}).get("enabled") is not True:
        raise ValueError("AI bootstrap must remain enabled during image updates")
