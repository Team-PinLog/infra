...
class AiImageCandidateTest(unittest.TestCase):
    def test_ai_image_matches_required_candidate(self):
        image = yaml.safe_load(VALUES.read_text(encoding="utf-8"))["image"]
        self.assertEqual(image["repository"], "ghcr.io/team-pinlog/ai")
        expected_tag = os.environ.get("AI_IMAGE_TAG")
        expected_digest = os.environ.get("AI_IMAGE_DIGEST")
        self.assertEqual(
            expected_tag is None,
            expected_digest is None,
            "AI_IMAGE_TAG and AI_IMAGE_DIGEST must be set together",
        )
