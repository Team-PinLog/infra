...
**작성 시점**: 2026-07-20 (초기 구축), **최근 갱신**: 2026-07-21
...
### 2.11 알림은 내부 상태와 외부 가용성을 분리

**결정**: 클러스터 내부 상태는 Prometheus → Alertmanager → 호스트 Sentinel
Receiver → Mattermost로 보낸다. 공개 HTTPS/TLS는 GitHub-hosted runner에서 별도로
검사한다.

**근거**: 단일 노드가 완전히 꺼지면 Prometheus, Alertmanager, Sentinel Receiver가
동시에 중단된다. 내부 알림 경로만으로는 그 장애를 알릴 수 없다. 외부 모니터는
같은 노드 밖에서 실행되어야 하며, 이 경우에만 deterministic한 고정 메시지로
Mattermost에 직접 전송한다.
...
| 모니터링 스택 (Prometheus·Alertmanager·Loki·Grafana·Alloy) | ~1.3Gi |
| **애플리케이션 가용분** | **~8.2Gi** |

**실측 (2026-07-21, 모니터링·Alertmanager 포함 / 서비스 0개)**:
...
- [`git-governance.md`](git-governance.md) — 브랜치 보호, PR·CI, 공급망 보안
- [`alerting.md`](alerting.md) — Alertmanager, Sentinel, 외부 모니터
